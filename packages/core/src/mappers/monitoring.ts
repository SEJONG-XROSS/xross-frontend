...
function buildEventDescription(e: EventResponse): string {
  switch (e.type) {
    case 'ENTER': return '매장에 고객이 입장했습니다.';
    case 'LOCATION_UPDATE': return '고객 위치가 업데이트되었습니다.';
    case 'SENSOR_TRIGGER': return '센서가 감지되었습니다.';
    case 'PICK': return `${e.product?.name ?? '상품'}을(를) 집었습니다.${e.freezer ? ` (${e.freezer.code})` : ''}`;
    ...
    case 'EXIT_LINE_CROSSED': return '고객이 퇴장 라인을 통과했습니다.';
    case 'LONG_STAY': return '고객이 장시간 체류 중입니다.';
    case 'FALL_DETECTED': return '낙상이 감지되었습니다.';
    case 'ALERT_SENT': return e.message ?? '알림이 발송되었습니다.';
  }
}
...
export function mapEventDetailsToVerification(details: EventDetailResponse[]): VerificationItem[] {
  return details
    ...
    .map((d) => {
      const mapping = SOURCE_MAP[d.source] ?? { logSource: 'system' as LogEntrySource, label: d.source };
      return {
  ...
  return [...details]
    ...
    .map((d) => {
      ...
      return {
        ...
        source: mapping.logSource,
        message: `${mapping.label}: ${typeLabel}`,
...
export function mapEventsToLogEntries(events: EventResponse[]): LogEntry[] {
  return [...events]
    .sort((a, b) => new Date(a.occurredAt).getTime() - new Date(b.occurredAt).getTime())
    .map((e) => {
      const mapping = SOURCE_MAP[e.source] ?? { logSource: 'system' as LogEntrySource, label: e.source };
      const severity = getSeverity(e.type);
      return {
        time: formatTime(e.occurredAt),
        source: mapping.logSource,
        message: `${mapping.label}: ${EVENT_TYPE_LABEL[e.type]}`,
        alert:
          severity === 'critical' ? ('critical' as const)
          : severity === 'warning' ? ('warning' as const)
          : undefined,
      };
    });
}
